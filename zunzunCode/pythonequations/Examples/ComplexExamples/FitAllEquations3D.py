#! /usr/bin/python
#    Version info: $Id: FitAllEquations3D.py 230 2010-06-30 20:20:14Z zunzun.com $
            ...
            if inspect.isclass(equationClass[1]):
                ...
                for extendedName in extendedNameList:
                    ...
                    if equation.polyfunctionalFlag == 1:
                        equationCount = 0
                        maxCoeffs = 2 # squared, arbitrary choice of maximum functions in a polyfunctional for this example
                        functionList = [] # make a list of function indices
                        for i in range(len(equation.EquationListForPolyfunctional[0])):
                            for j in range(len(equation.EquationListForPolyfunctional[1])):
                                if [i,j] not in functionList:
                                    functionList.append([i,j])

                        for coeffCount in range(1, maxCoeffs+1):
                            xComboList = UniqueCombinations(functionList, coeffCount)
                            for k in xComboList:
                                equation.Polyfun3DFlags = k
                                SetParametersAndFit(equation, resultList)
                                equationCount += 1
                                if (equationCount % 5000) == 0:
                                    print '    ', equationCount, '...'
                        print '    ', equationCount, 'equations fitted'
